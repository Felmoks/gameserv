        ...
        match token {
            SERVER => {
                ...
                match self.server.accept() {
                    ...
                    Ok(None) => {
                        println!("the server wasn't actually ready");
                    }
                    Err(e) => {
...
fn main() {
    ...
    event_loop.register(&server, SERVER, mio::EventSet::readable(), mio::PollOpt::edge()).unwrap();
    
    println!("running pingpong server");
    event_loop.run(&mut Pong { server: server }).unwrap();
}
